...
def parse_node(data: bytes, pos: int) -> tuple:
    ...
    # Sanity checks to prevent infinite loops / hangs on garbage data
...
def main():
    ...
    # Let's compare 0x001AB084 (size 541,397 + 12 = 541,409)
    patched_offset = 0x001AB084
    patched_size = 541409
    
    print(f"[*] Reading loaded Patched root at PS2 0x{patched_offset:X}...")
    pat_raw = read_remote_mem(hProcess, eemem + patched_offset, patched_size)
    if pat_raw:
        pat_root, _ = parse_node(pat_raw, 0)
        print("\n==================================================")
        print("  LOADED PATCHED HYBRID MODEL TREE IN RAM")
        print("==================================================")
        dump_tree(pat_root)
