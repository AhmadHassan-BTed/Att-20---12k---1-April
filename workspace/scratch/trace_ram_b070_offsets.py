...
def read_remote_mem(proc_handle, address, size):
    ...
    if not ok:
        print(f"ReadProcessMemory at 0x{address:X} size {size} failed with error {ctypes.get_last_error()}")
...
def find_eemem_base(proc_handle, pid, exe_name):
    ...
    print(f"Module base address: {base_addr}")
    if not base_addr:
        return None
...
def main():
    ...
    # Try OpenProcess with escalation
    ...
    if not hProcess:
        print(f"[-] OpenProcess failed with error {ctypes.get_last_error()}")
        return
        
    print(f"Opened process handle: {hProcess}")
    eemem = find_eemem_base(hProcess, pid, name)
    print(f"EEmem: {eemem}")
    if not eemem:
        print("[-] Failed to find EEmem base")
        return
