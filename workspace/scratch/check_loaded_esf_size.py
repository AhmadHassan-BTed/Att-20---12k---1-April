...
def main():
    ...
    # Let's read header
    hdr = read_remote_mem(hProcess, eemem + 0x0186CA40, 32)
    print("=== ESF HEADER IN RAM ===")
    print(f"Magic:       {hdr[0:4]}")
    print(f"Version:     {struct.unpack_from('<I', hdr, 4)[0]}")
    print(f"Constant:    0x{struct.unpack_from('<I', hdr, 8)[0]:08X}")
    print(f"Header Size: {struct.unpack_from('<I', hdr, 16)[0]}")
    
    # Let's read root node header
    root_hdr = read_remote_mem(hProcess, eemem + 0x0186CA40 + 32, 12)
    type_id = struct.unpack_from('<I', root_hdr, 0)[0]
    data_size = struct.unpack_from('<I', root_hdr, 4)[0]
    child_count = struct.unpack_from('<I', root_hdr, 8)[0]
    print("\n=== ROOT NODE IN RAM ===")
    print(f"Type ID:     0x{type_id:X}")
    print(f"Data Size:   {data_size:,} bytes")
    print(f"Child Count: {child_count}")
    print(f"Total Size:  {data_size + 12:,} bytes")
