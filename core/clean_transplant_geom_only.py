...
def main():
    ...
    print("=" * 80)
    print("  PS2 CLEAN TRANSPLANT: GEOMETRY-ONLY INJECTION (PREVENT SKELETON OVERFLOW)")
    print("=" * 80)
    ...
    for t in targets:
        ...
        if van_geom and fro_geom_idx is not None:
            ...
            print(f"      -> Swapped entire 0x02610 Geometry Node (Preserving Vanilla DMA Chains)")
        else:
            print("      -> [!] Error: Geometry Node missing!")
            
        # 1.5 Bounding Box Protection
        # We MUST copy the bounding box and sphere from the Vanilla root node's inline_data
        # into the Graft (Frontiers) root node's inline_data to prevent engine culling.
        # The bounding limits are stored at offset 0x20 and sphere at 0x38 in the root node's inline data.
        ...
        # Determine which node holds the inline_data (root or first child)
        van_hdr_data = van_node['children'][0]['inline_data'] if van_node['children'][0]['child_count'] == 0 else van_node['inline_data']
        fro_hdr_data = graft_root['children'][0]['inline_data'] if graft_root['children'][0]['child_count'] == 0 else graft_root['inline_data']
        ...
        if van_bb_off is not None and fro_bb_off is not None:
            ...
            if graft_root['children'][0]['child_count'] == 0:
                graft_root['children'][0]['inline_data'] = bytes(c_fro)
            else:
                graft_root['inline_data'] = bytes(c_fro)
                
            print(f"      -> Injected Bounding Box from Vanilla: X({van_bb[0]:.2f} to {van_bb[3]:.2f})")
        else:
            print("      -> [!] Warning: Could not cleanly locate bounding box for transplant.")

        # 2. KEEP 0x0B070 (Skeleton) and 0x02800 (Bone Matrices) from Frontiers!
        print(f"      -> Retained Frontiers 0x0B070 Skeleton Node (Prevents EE Buffer Overflow)")
        print(f"      -> Retained Frontiers 0x02800 Bone Matrix Node (Matches Skeleton)")
    ...
    print("\n[+] Success! MANUAL_PATCH.iso is ready with Geometry-Only Injection.")
